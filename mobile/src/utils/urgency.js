...
//
// Ported from the web app: `color` used to hold CSS custom-property strings
// ('var(--status-rejected)'), which are meaningless in RN, so they now resolve
// to real values from theme.js.
...
const urgencyFromRatio = (ratio) => {
  if (ratio >= HIGH_THRESHOLD) {
    return { level: 'High Urgency', label: 'High Urgency', color: color.statusRejected };
  }
  if (ratio >= MEDIUM_THRESHOLD) {
    return { level: 'Medium Urgency', label: 'Medium Urgency', color: color.statusPending };
  }
  return { level: 'Standard Urgency', label: 'Standard Urgency', color: color.accent };
};
